...
#include "esp_system.h"
// #include "esp_clk.h"
// Arduino ESP32 timer variables
hw_timer_t *us_timer = NULL;
const int timer_divider = 80;  // Divider for 1MHz (1us) resolution with 80MHz APB clock
...
int luat_mcu_set_clk(size_t mhz) {
    // Arduino ESP32 doesn't directly expose esp_pm_configure
    // We can use setCpuFrequencyMhz() from the ESP class instead
    if (mhz == 240 || mhz == 160 || mhz == 80 || mhz == 40 || mhz == 20 || mhz == 10) {
      return setCpuFrequencyMhz(mhz) ? 0 : -1;
    } else {
...
void luat_mcu_us_timer_init() {
    // Initialize timer with 1MHz (1us) resolution
    us_timer = timerBegin(1000000);  // Timer 0, divide by 80 (for 1MHz), count up
    
    // No need for an interrupt if you're just using it as a time source
    // If you need an interrupt handler, add it with timerAttachInterrupt()
