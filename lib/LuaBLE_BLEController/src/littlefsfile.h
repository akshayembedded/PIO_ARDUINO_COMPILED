#ifndef LITTLEFSFILE_H
#define LITTLEFSFILE_H
#include <Arduino.h>
#include <spiffs.h>
#include <FS.h>
...
inline ErrorCode initFS();
inline ErrorCode formatFS();
 ...
 inline String errorCodeToString(ErrorCode errorCode)
  {
    switch (errorCode)
    {
      ...
      case FS_FORMAT_FAILED:
        return "FS_FORMAT_FAILED";
      case FS_FORMAT_SUCCESS:
        return "FS_FORMAT_SUCCESS";
      default:
 ...
 inline ErrorCode initFS()
  {
    ...
    {
      Serial.println("An Error has occurred while mounting LitteFS");
      return FS_NOT_INITIALIZED;
    }
    Serial.println("LitteFS initialized successfully");
    return FS_INITIALIZED;
  }
 ...
 inline ErrorCode createFile(const std::string &filename, size_t size)
  {
    ...
    currentFile = LittleFS.open(filename.c_str(), "w");
    if (!currentFile)
    {
      return FILE_OPEN_FAILED;
    }
 ...
 inline void flushBuffer()
  {
    if (bufferPos > 0)
    {
      ...
      if (bytesWritten != bufferPos)
      {
        currentFile.flush();
        currentFile.close();
        currentFile = LittleFS.open(currentFileName.c_str(), "a");
        if (!currentFile)
        {
          return;
        }
...
inline  ErrorCode closeFile()
 ...
 inline ErrorCode renameFile(const std::string &oldFilename, const std::string &newFilename)
  {
    if (!LittleFS.exists(oldFilename.c_str()))
    {
      return FILE_NOT_FOUND;
    }

    if (LittleFS.rename(oldFilename.c_str(), newFilename.c_str()))
    {
      return FILE_OK;
    }
    else
