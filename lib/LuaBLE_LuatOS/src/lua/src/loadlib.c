  ...
  if (stat) {  /* module loaded successfully? */
    ...
    return luaL_error(L, "error loading module '%s' from file '%s':\n\t%s",
      ...
      #define LUAT_MODULE_SEARCH_PATH   "/%s.luac", "/%s.lua", \
        "/luadb/%s.luac", "/luadb/%s.lua",\
        "/lua/%s.luac", "/lua/%s.lua",\
          "/littlefs/%s.luac", "/littlefs/%s.lua"
      #endif
     #endif
...
char custom_search_paths[8][24] = {0};

int luat_search_module(const char* name, char* filename) {
  int index = 0;
    if (filename == NULL) {
    LLOGI("luat_search_module: filename is NULL");
    return -1;
  }
...
size_t path_count = sizeof(search_paths) / sizeof(search_paths[0]);
  for (size_t index = 0; index < path_count; index++) {
    ...
    LLOGI("luat_search_module: filename is %s", filename );
    if (luat_fs_fexist(filename)) return 0;
    filename[0] = 0x00;
  }
  return -1;
